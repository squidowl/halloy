...
pub trait ChannelsContext {
    fn is_focused(&self, server: &Server, channel: &target::Channel) -> bool;
    fn is_open(&self, server: &Server, channel: &target::Channel) -> bool;
    ...
    fn channel_entries(
        ...
        Entry::channel_list(
            self.is_open(server, channel),
            self.is_focused(server, channel),
        )
    }
...
pub enum Context<'a> {
    ...
    NotSentMessage(&'a DateTime<Utc>, &'a history::Id),
...
impl<'a> Context<'a> {
    pub fn link(
        link: &'a message::Link,
        into_user_context: Option<impl Fn(&'a User) -> UserContext<'a>>,
        into_url_context: Option<impl Fn(&'a str) -> UrlContext<'a>>,
        into_channel_context: Option<
            impl Fn(&'a Server, &'a target::Channel) -> ChannelContext<'a>,
        >,
    ) -> Option<Context<'a>> {
        match link {
...
impl Entry {
    ...
    pub fn channel_list(is_open: bool, is_focused: bool) -> Vec<Self> {
        let mut entries = vec![Entry::CopyChannel];

        if !is_open {
            entries.push(Entry::OpenChannelInNewPane);
            entries.push(Entry::OpenChannelInNewWindow);
        }

        if !is_focused {
            entries.push(Entry::OpenChannelInFocusedPane);
        }
    ...
    pub fn as_label_and_message(
        ...
        match (self, context) {
            ...
            (
                ...
                message.map(|message| {
                    Message::HidePreview(message.history_id, url.to_string())
                }),
            )),
            (
                Entry::ShowPreview,
                ...
                message.map(|message| {
                    Message::ShowPreview(message.history_id, url.to_string())
                }),
            )),
            (
                Entry::CopyChannel,
...
pub enum Message {
    ...
    CopyText(String),
    OpenUrl(String),
    HidePreview(history::Id, String),
    ShowPreview(history::Id, String),

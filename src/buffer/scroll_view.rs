...
impl State {
    pub fn new(
        ...
        let message_count = step_messages(4.0 * pane_size.height, config);
    ...
    pub fn update(
        ...
        match message {
            ...
            Message::ScrollTo(keyed::Hit {
                ...
                let reverse_inset = if matches!(key, keyed::Key::Divider) {
                    0.0
                } else {
    ...
    fn adjusted_message_count(
        ...
        if self.content_size.height < 4.0 * self.pane_size.height
            && has_more_messages
        {
            count = count.saturating_add(step);
        } else if self.content_size.height > 8.0 * self.pane_size.height
            && count > 2 * step
        {

...
use data::{Config, Server, channel_discovery_manager, message, target};
use iced::widget::{
...
pub fn view<'a>(
    ...
    let selected_server = state.selected_server.as_ref();
        ...
        column![
            row![
                ...
                text_input("Search..", &state.search_query)
                    .on_input_maybe(selected_server.map(|_| |query| Message::SearchQuery(query)))
            ]
            .spacing(8)

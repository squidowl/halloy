...
impl<'a> ChannelQueryLayout<'a> {
    ...
    fn previews_enabled(&self, message: &message::MessageDisplay) -> bool {
    ...
    fn not_sent(&self, message: &message::Message) -> bool {
        self.confirm_message_delivery
            && let message::Direction::Sent { command } = &message.direction
    ...
    fn format_condensed_message(
        ...
        let link =
            message::Link::ExpandMessage(message.time.utc, message.hash, None);
    ...
    fn content_on_new_line(&self, message: &message::MessageDisplay) -> bool {
        ...
        use message::Source;
        matches!(
            (message.source, self.config.buffer.nickname.alignment,),
            ...
            if let message::Content::Fragments(fragments) = reply {
                let loaded: Vec<(usize, &Preview)> = fragments
                    ...
                    .filter(|(_, url)| {
                        !self
                            .history
                            .is_preview_hidden(&kind, reply_hash, time, url)
                    })

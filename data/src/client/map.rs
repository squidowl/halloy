...
impl Map {
    ...
    pub fn receive(
        ...
        if let Some(client) = self.client_mut(server) {
            client.receive(message, storage, buffers_context, config)
        } else {
            Ok(Vec::default())
        }
    }
    ...
    pub fn send(
        ...
        self.client_mut(buffer.as_server())
    ...
    pub fn contains_query(
        ...
        self.client(server)
            .is_some_and(|c| c.querymap.contains_key(query))
    }
    ...
    pub fn request_older_chathistory(
        ...
        if let Some(client) = self.client_mut(server) {
            client.request_older_chathistory(target, priority, storage);
        }
    }
                    ...
                    if let State::Ready(client) = &state {
                        Some((
                            ...
                            ServerContextSnapshot {
                                ...
                                message_reference_types: client
...
impl ClientsContext for Map {
    fn get_server_casemapping(
        &self,
        server: &Server,
    ) -> Option<isupport::CaseMap> {
        self.client(server).map(Client::casemapping)
    }
    ...
    fn get_server_chathistory_message_reference_types(
        &self,
        server: &Server,
    ) -> &[isupport::MessageReferenceType] {
        if let Some(message_reference_types) = self
            .client(server)
...
pub struct ClientsContextSnapshot {
    server_contexts: HashMap<Server, ServerContextSnapshot>,
}

struct ServerContextSnapshot {
...
impl ClientsContext for ClientsContextSnapshot {
    fn get_server_casemapping(
        ...
        self.server_contexts
            .get(server)
            .map(|server_context| server_context.casemapping)
    }
    ...
    fn get_maybe_server_casemapping_or_default(
        ...
        server
            .and_then(|server| self.server_contexts.get(server))
            .map(|server_context| server_context.casemapping)
            .unwrap_or_default()
    }
    ...
    fn get_server_chathistory_message_reference_types(
        ...
        if let Some(message_reference_types) = self
            .server_contexts
            .get(server)
            .map(|server_context| &server_context.message_reference_types)
        {
            message_reference_types
        } else {
            &[]

...
/// RPL_ENDOFMOTD, ERR_NOMOTD, or JOIN);  if the message does not have a time
/// tag provided by the server, then the message::Time generated from local time
/// will be shifted into the future.  If the resultant time is ahead of the
/// server's clock, then deduplication should take care of any duplicate
/// messages we receive.

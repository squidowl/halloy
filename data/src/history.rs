...
impl Kind {
    ...
    pub fn from_upstream_buffer(buffer: buffer::Upstream) -> Self {
        match buffer {
    ...
    pub fn from_buffer(buffer: Buffer) -> Option<Self> {
        match buffer {
            Buffer::Upstream(upstream) => {
                Some(Self::from_upstream_buffer(upstream))
            }
...
impl Kind {
    pub fn as_ref(&self) -> KindRef<'_> {
        match self {
            Kind::Server(server) => KindRef::Server(server),
            Kind::Channel(server, channel) => KindRef::Channel(server, channel),
            Kind::Query(server, query) => KindRef::Query(server, query),
            Kind::Logs => KindRef::Logs,
            Kind::Highlights => KindRef::Highlights,
            Kind::ChannelMonitor => KindRef::ChannelMonitor,
        }
    }
...
impl KindRef<'_> {
    pub fn as_server(&self) -> Option<&Server> {
        match self {
            KindRef::Server(server)
            | KindRef::Channel(server, _)
            | KindRef::Query(server, _) => Some(server),
            KindRef::Logs | KindRef::Highlights | KindRef::ChannelMonitor => {
                None
            }
        }
    }
...
impl<'a> From<&'a buffer::Upstream> for KindRef<'a> {
    fn from(upstream_buffer: &'a buffer::Upstream) -> Self {
        match upstream_buffer {
            buffer::Upstream::Server(server) => Self::Server(server),
            buffer::Upstream::Channel(server, channel) => {
                Self::Channel(server, channel)
...
impl From<KindRef<'_>> for Kind {
    fn from(value: KindRef<'_>) -> Self {
        match value {
            KindRef::Server(server) => Kind::Server(server.clone()),
            KindRef::Channel(server, channel) => {
                Kind::Channel(server.clone(), channel.clone())
            }
            KindRef::Query(server, nick) => {
                Kind::Query(server.clone(), nick.clone())
            }
            KindRef::Logs => Kind::Logs,
            KindRef::Highlights => Kind::Highlights,
            KindRef::ChannelMonitor => Kind::ChannelMonitor,
        }
    }
}

impl Equivalent<Kind> for KindRef<'_> {
    fn equivalent(&self, kind: &Kind) -> bool {
        match self {
            KindRef::Server(server_ref) => {
                if let Kind::Server(server) = kind
                    && server == *server_ref
                {
                    true
                } else {
            ...
            KindRef::Channel(server_ref, channel_ref) => {
            ...
            KindRef::Query(server_ref, query_ref) => {
                if let Kind::Query(server, query) = kind
                    && server == *server_ref
                    && query == *query_ref
                {
                    true
            ...
            KindRef::Logs => matches!(kind, Kind::Logs),
            KindRef::Highlights => matches!(kind, Kind::Highlights),
            KindRef::ChannelMonitor => matches!(kind, Kind::ChannelMonitor),
        }
    }
}
...
/// `date_time`. If all messages are after the specified `date_time` then `0`
/// will be returned.
pub fn position_last_message_before_or_at_date_time<M>(
    messages: &[M],
...
where
    M: message::Searchable,
{
    position_first_message_after_date_time(messages, date_time)
        .saturating_sub(1)
}

/// The position of the first message with time after the specified `date_time`.
/// If all messages are at or before the specified `date_time` then
/// `messages.len()` will be returned.
pub fn position_first_message_after_date_time<M>(
    messages: &[M],
    date_time: &DateTime<Utc>,
) -> usize
where
    M: message::Searchable,
{
    ...
    if let Some(from_start_index) = messages[start_index..]
        .iter()
        .position(|message| message.time().utc > *date_time)
    {
        start_index + from_start_index
    } else {
        messages.len()
    }
}

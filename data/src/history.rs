...
use crate::message::{self, MessageReferences, MsgId, Reaction, Source};
...
pub async fn overwrite(
    ...
    if messages.is_empty() {
        ...
        return metadata::save(kind, messages, read_marker, reactions.clone()).await;
    }
    ...
    let reactions_for_messages: HashMap<MsgId, Vec<Reaction>> = reactions
        .iter()
...
pub async fn append(
    ...
    let mut all_reactions = loaded.metadata.reactions;
...
impl History {
    ...
    fn reactions_mut(&mut self) -> &mut HashMap<MsgId, Vec<Reaction>> {
        match self {

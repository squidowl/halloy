...
pub use self::map::{ClientsContext, Map, State, Status};
...
impl Client {
    ...
    pub fn update_config(
        ...
        let mut events = config
            ...
            .filter_map(|query| {
                ...
                .ok()
                .map(|query| Event::ShowInSidebar(Target::Query(query)))
            })
            .collect::<Vec<_>>();
    ...
    fn resend_privmsg_or_notice(
        ...
        message.as_command().and_then(|command| {
            let multiline = if let Some((target, text, batch_kind)) =
                ...
                if text.lines().count() > 1 {
                    true
                } else if let Some(multiline_limits) = self.multiline_limits() {
                    let multiline_concat_bytes = multiline_limits.concat_bytes(
                        self.relay_bytes(),
                        batch_kind,
                        target,
            ...
            // TODO: Use Client::handle to create message::MessageWithContext
            // instead of Irc::messages.  Similarly, return
            // message::MessageWithContext by Client::send and
            // Client::send_multiline_batch in place of LabeledResponseContext,
            // utilizing Client::handle.

            let channel_users =
                buffer.as_channel().and_then(|channel| self.users(channel));

            let mut user = User::from(self.nickname().to_owned());

            if let Some(channel_users) = channel_users {
                user = channel_users.resolve(&user).cloned().unwrap_or(user);
            }
            ...
            if let Some(mut message) = command
                .messages(
                    user,
                    channel_users,
                    buffer.as_server(),
                    ...
                    self.statusmsg(),
                    self.casemapping(),
                    self.capabilities.acknowledged(Capability::EchoMessage),
                    reroute_rules,
                    Some(buffer),
                )
                .and_then(|messages| messages.into_iter().next())
                && let Some(mut encoded) =
                    proto::Command::try_from(command.clone())
                        .ok()
                        .map(proto::Message::from)
                        .map(message::Encoded::from)
            {
                let labeled_response_context = if multiline {
                    self.send_multiline_batch(
                        buffer,
                        vec![encoded],
                        priority,
                        reply_id,
                    ...
                    self.send(Some(buffer), encoded, priority)
                };

                message.reply_to = reply_id.cloned();
    ...
    fn receive(
        ...
        let events =
            self.handle(message, None, storage, buffers_context, config)?;
    ...
    fn handle(
        ...
        match &message.command {
            ...
            _ if batch_tag.is_some() => {
                if let Some(batch_tag) = batch_tag.as_ref() {
                        ...
                        .and_then(|batch| batch.kind.as_ref())
                    {
                        ...
                        Some(BatchKind::ZncPlayback(target)) => self
                            .handle_znc_playback(
                                message,
                                target.clone(),
                                storage,
            ...
            Command::INVITE(invitee, _) => {
                ...
                return Ok(self
                    .process_message(message, storage, buffers_context, config)
                    .into_iter()
                    .map(Event::Message)
                    .collect());
            }
                ...
                return Ok(vec![Event::Broadcast(
                    broadcast::BroadcastWithContext {
                        in_channels: broadcast::Channels::Vec(
                            self.user_channels(old_user.nickname()),
                        ),
                        in_server: ourself,
            ...
            Command::JOIN(channel, accountname) => {
                ...
                if user.nickname() == self.nickname() {
                    ...
                    return Ok(vec![Event::ShowInSidebar(target)]);
                } else if let Some(channel) =
            ...
            Command::KICK(channel, victim, reason) => {
                ...
                if let Ok(channel) = target::Channel::parse(
                        ...
                        == self.nickname().as_normalized_str()
                    {
                        ...
                        return Ok(vec![Event::Broadcast(
                            broadcast::BroadcastWithContext {
                                in_channels: broadcast::Channels::Vec(vec![
                                    channel,
                                ]),
                                in_server: true,
                ...
                match &finished.kind {
                    ...
                    Some(BatchKind::ChathistoryTargets) => {
                            ...
                            continuation_subcommand
                        {
                            ...
                            storage.update_last_received_chathistory_targets(
                                &self.server,
                                Utc::now(),
                            );
                        }
    ...
    fn handle_chathistory(
        ...
        match &message.command {
            ...
            Command::REDACT(_, _, _) => self
                ...
                .map(Event::Redaction)
                .collect(),
            Command::NICK(_) | Command::JOIN(_, _) | Command::PART(_, _) => {
                self.process_message(message, storage, buffers_context, config)
            ...
            Command::QUIT(_) => self
                .process_message(message, storage, buffers_context, config)
                .map(|message| message.with_target(target.into()))
                .map(message::MessageWithContext::into_historical)
                .into_iter()
                .map(Event::Message)
                .collect(),

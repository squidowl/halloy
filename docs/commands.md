...
| `upload`        |              | Upload a file via `draft/FILEHOST` with an optional path[^11]                            |
...
[^11]: Requires the server to advertise `draft/FILEHOST` support, or [`filehost.override_url`](./configuration/servers#filehost) to be set.
